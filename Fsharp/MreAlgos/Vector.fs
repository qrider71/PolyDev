...
type Vector<'T>(elements: array<'T>, add':'T -> 'T -> 'T, sub':'T -> 'T -> 'T, mul':'T -> 'T -> 'T) =
    member this.v = elements

    static member (+) (v1: Vector<'T>, v2: Vector<'T>) = v1.add(v2)
    member this.add (v:Vector<'T>) = Vector(this._add (this.v, v.v), add',sub', mul')
    member private this._add (v1, v2) = Array.map2 add' v1 v2

    static member (-) (v1: Vector<'T>, v2: Vector<'T>) = v1.sub(v2)
    member this.sub (v:Vector<'T>) = Vector(this._sub (this.v, v.v), add',sub', mul')
    ...
    member this.mul (c:'T) = Vector(this._cmul (this.v, c), add',sub', mul')
